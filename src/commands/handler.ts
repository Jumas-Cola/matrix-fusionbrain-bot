...
import { runImagineCommand } from './imagine';
...
export const COMMAND_PREFIX = '!imagine';
...
export default class CommandHandler {
  ...
  private async onMessage(roomId: string, ev: any) {
    ...
    try {
      LogService.info('CommandHandler', JSON.stringify(args));
      if (args[0].length > 0) {
        return runImagineCommand(
          roomId,
          event,
          args,
          this.client,
